...
interface CityAuthorProps {
  author: CityData['author']
}

export default function CityAuthor({ author }: CityAuthorProps) {
  const credentials = [
    { icon: Briefcase, label: '3+ tahun fokus web development & digital growth' },
    { icon: Award, label: '30+ proyek website bisnis & landing page terkirim' },
    { icon: MapPin, label: author.credentialLocation },
    { icon: ShieldCheck, label: 'Kontrak jelas, komunikasi langsung, tanpa perantara' },
  ] as const

  return (
    <section aria-labelledby="author-heading" className="py-20 lg:py-32 bg-white">
        ...
        <div className="grid lg:grid-cols-[auto_1fr] gap-10 lg:gap-16 items-start">
          <div className="flex-shrink-0">
            <div className="relative w-56 h-72 lg:w-72 lg:h-96 rounded-3xl overflow-hidden bg-[#F3F4F6] border border-gray-200 shadow-sm">
              <Image
                ...
                alt="Foto Noviyanto — Web Developer & Digital Growth Partner"
                fill
          ...
          <div>
            ...
            <div className="space-y-4 text-base lg:text-lg text-[#6B7280] leading-relaxed">
              <p>
                Saya membangun website untuk bisnis yang serius ingin tumbuh — bukan sekadar punya
                alamat online. Latar belakang saya adalah web development dan digital marketing, jadi
                setiap halaman yang saya buat dirancang dengan satu pertanyaan utama: bagaimana
                halaman ini bisa mengubah pengunjung jadi calon klien?
              </p>
              <p>
                Saya bekerja sebagai individu profesional, bukan agency. Artinya tidak ada
                salesperson, tidak ada project manager perantara, dan tidak ada tim freelance yang
                berganti-ganti di belakang layar. Anda berkomunikasi langsung dengan orang yang
                menulis kode, menyusun copy, dan mengatur strategi SEO halaman Anda.
              </p>
              <p>
                Bagi saya, reputasi lebih penting daripada volume. Setiap proyek yang saya ambil
                adalah proyek yang saya yakin bisa saya bawa ke hasil yang nyata.
              </p>
            </div>
            ...
            <ul className="mt-8 grid sm:grid-cols-2 gap-3">
              {credentials.map(({ icon: Icon, label }) => (
                ...
                >
                  ...
                  <p className="text-sm text-[#111827] font-medium leading-snug pt-1.5">{label}</p>
                </li>
              ))}
            </ul>
